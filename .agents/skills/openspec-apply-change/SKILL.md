...
**输入**：可选地指定变更名称（例如 `$openspec-apply-change (Codex) or /openspec-apply-change (other agents) add-auth`）。若省略，检查能否从对话上下文推断。若模糊或歧义，你必须提示用户从可用变更中选择。
...
All tasks complete! 你可以用 `$openspec-archive-change (Codex) or /openspec-archive-change (other agents)` 归档此变更。
```
