...
function copyButton(env: unknown, value: string, label: string) {
  ...
  return `<button type="button" class="segment-copy" data-copy-index="${index}" aria-label="复制${label} ${index + 1}" title="复制${label}"><span class="pi pi-copy" aria-hidden="true"></span></button>`;
}
...
md.renderer.rules.inline_math = (tokens, index, _options, env) => `<span class="inline-formula">${math(tokens[index]!.content, false)}${copyButton(env, tokens[index]!.content, '公式')}</span>`;
md.renderer.rules.block_math = (tokens, index, _options, env) => `<div class="rich-segment formula-block">${copyButton(env, tokens[index]!.content, '公式')}<div class="formula-scroll">${math(tokens[index]!.content, true)}</div></div>`;
md.renderer.rules.fence = (tokens, index, _options, env) => `<div class="rich-segment code-block">${copyButton(env, tokens[index]!.content, '代码')}<pre><code>${escape(tokens[index]!.content)}</code></pre></div>`;
md.renderer.rules.code_block = md.renderer.rules.fence;
md.renderer.rules.paragraph_open = (tokens, index, _options, env) => tokens[index]!.hidden ? '' : `<div class="rich-segment explanation-block">${copyButton(env, tokens[index + 1]?.content ?? '', '说明')}<p>`;
md.renderer.rules.paragraph_close = (tokens, index) => tokens[index]!.hidden ? '' : '</p></div>';
md.renderer.rules.image = (tokens, index) => `<span class="image-placeholder">[图片：${escape(tokens[index]!.content)}；原型不自动加载外部图片]</span>`;
