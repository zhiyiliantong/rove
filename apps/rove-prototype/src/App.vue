...
const section = computed(() => route.value.split('/')[1] || 'sessions');
...
<template>
  <div class="app-shell" :data-platform="platform" :class="{ 'mobile-platform': mobilePlatform }">
    ...
    <nav class="bottom-nav" aria-label="移动主导航"><a v-for="link in links" :key="link.id" :href="`#/${link.id}`" :class="{ active: navSection === link.id }" :aria-current="navSection === link.id ? 'page' : undefined"><i aria-hidden="true" :class="`pi ${link.icon}`"/><span>{{ link.label }}</span></a></nav>
    <ModelDialog/><NetworkDialogs/>
