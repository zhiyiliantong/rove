...
if (!expression) throw new Error('Usage: node scripts/android-webview-eval.mjs <expression> [new-result.json]');
const pages = await (await fetch('http://127.0.0.1:19222/json')).json();
const page = pages.find(p => {
  try { const url=new URL(p.url); return url.origin==='http://tauri.localhost'&&url.pathname==='/'&&p.title.startsWith('Rove'); }
  catch { return false; }
});
...
ws.addEventListener('message', event => {
  ...
  if (reply.error || reply.result.exceptionDetails) {
    ...
    const output = JSON.stringify(reply.result.result.value, null, 2);
